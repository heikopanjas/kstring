...
2. **Two Representations**:
   - **Short strings (≤12 chars)**: 32-bit length + 96-bit inline content
   - **Long strings (>12 chars)**: 32-bit length + 32-bit prefix + 2-bit storage class + 62-bit pointer
...
2. **API Functions** (implemented):
   - `KStringCreate(pStr, Size)` - Secure string creation with explicit byte count
   - `KStringCreatePersistent(pStr, Size)` - Persistent string with explicit byte count
   - `KStringCreateTransient(pStr, Size)` - Transient string with explicit byte count
...
**Note**: While the internal structure uses `uint32_t` to maintain the critical 16-byte size constraint from the German String specification, all public API functions use `size_t` for size parameters and return values to follow standard C conventions.
...
KString KStringCreate(const char* pStr, const size_t Size);           // Size = count of bytes
KString KStringCreatePersistent(const char* pStr, const size_t Size); // Size = count of bytes
KString KStringCreateTransient(const char* pStr, const size_t Size);  // Size = count of bytes
...
size_t KStringSize(const KString kstr);       // Get size (always fast)
...
2. **Size First**: Always check size in comparisons before accessing content
3. **Prefix Optimization**: Use 4-byte prefix for fast long string comparisons
4. **Immutable Design**: Never modify KString content after creation
5. **Storage Class Awareness**: Handle different lifetimes appropriately
...
static inline bool is_short_string(KString kstr) {
    return kstr.length <= KSTRING_MAX_SHORT_LENGTH;
}
```
...
1. Compare size (32-bit integer comparison)
2. If different sizes → not equal
3. Compare prefix (32-bit integer comparison of first 4 chars)
4. If different prefixes → not equal
5. Only then compare full content if needed
...
- Internal structure may use `uint32_t` for space efficiency (16-byte constraint)
- Convert between `uint32_t` (internal) and `size_t` (API) as needed
- Check for overflow when converting from `size_t` to `uint32_t`
